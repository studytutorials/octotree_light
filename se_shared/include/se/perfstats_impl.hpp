...
inline double PerfStats::Stats::mergeIter(const std::vector<double>& iter_data_vec,
                                          const Type                 type) {
  switch (type) {
    case COUNT:
...
inline std::string PerfStats::createHeaderString() {
  ...
  for (const auto& type : header_order_) {
    for (const auto& o : order_) {
      ...
      if (s == stats_.end() ||
          s->second.detailed_ > include_detailed_) { // if include_detailed == false (0) only include non detailed i.e. detailed == false (0)
        continue;
      }
...
inline void PerfStats::writeSummaryToOStream(std::ostream& ostream,
                                             bool          include_iter_data) {
